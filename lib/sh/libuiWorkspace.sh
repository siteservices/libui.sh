...
ValidateWorkspace () { # [-n|-w]
  ...
  if [[ -z "${WORKSPACE}" ]]
  then
    test -f "${_WS_wsfile}" && source "${_}" || WORKSPACE="${PWD}"
  fi

  ${_M} && _Trace 'Validate WORKSPACE. (%s)' "${WORKSPACE}"
  if [[ -n "${WORKSPACE}" ]]
  then
    ${_M} && _Trace 'Check for WORKSPACE directory. (%s)' "${WORKSPACE}"
    ...
    ${_M} && _Trace 'WORKSPACE undefined.'
    return 1
  fi

  ${_M} && _Trace 'ValidateWorkspace return. (%s)' "${_WS_rv}"
  return ${_WS_rv}
}
